...
def entry(self):
    """
     If hdbscan is not present in the path, run train() to train the model for the prediction.
     Calls predict function for every 1 second(for now as we are using simulated data).
    """
    if not os.path.isfile('/tmp/ad/hdbscan'):
        train()
...
def predict(self):
    """
     Read the input csv file that has 100 UEIDs with both normal and anomalous data.
